...
- **Minimal Disruption**: DNS is only restarted when the file actually changes, avoiding unnecessary service interruptions
...
Verify the file is reachable before continuing:

```bash
curl http://<primary-pihole-ip>/custom.list
```

### 2. Install script into secondary server(s)

SSH into the secondary Pi-hole server and download the script directly:

```bash
ssh pi@<secondary-pihole>
sudo wget -O /usr/local/bin/sync-from-web.sh \
  https://raw.githubusercontent.com/alexd-gh/Pi-Hole-custom.list-Synchronizer/main/sync-from-web.sh
...
Run `sudo crontab -e` and add the following line to run every 15 minutes:

```
*/15 * * * * /usr/local/bin/sync-from-web.sh >/dev/null 2>&1
```

> **Note:** Stdout and stderr are suppressed in this cron entry. All script activity is still logged to syslog — use `sudo journalctl -t "Pi-Hole Custom DNS Synchronizer"` to monitor execution and catch failures.
...
- **Primary Server**: Bash, lighttpd (included with Pi-hole), write access to `/var/www/html/`
- **Secondary Server(s)**: Bash, `wget` or `curl`, `sudo` privileges, network access to primary on port 80
